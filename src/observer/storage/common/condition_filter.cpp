...
RC DefaultConditionFilter::init(Table &table, const ConditionSqlNode &condition)
{
  ...
  {
    left.is_attr = true;
    const FieldMeta *field_left = table_meta.field(field_expr_left->table_name());
    if (nullptr == field_left) {
      LOG_WARN("No such field in condition. %s.%s", table.name(), field_expr_left->table_name());
  ...
  {
    right.is_attr = true;
    const FieldMeta *field_right = table_meta.field(field_expr_right->table_name());
    if (nullptr == field_right) {
      LOG_WARN("No such field in condition. %s.%s", table.name(), field_expr_right->table_name());
      return RC::SCHEMA_FIELD_MISSING;
    }
